...
public class CreatComponents {
	...
	/** Use the random tasks from the workflow template to get the VM list, each task bind to a VM. */
	 public  void createVMList() throws IOException, ClassNotFoundException{
		...
		/**
		//Get the VM list CPU utilization into txt files
		for(int i=0;i<vmList.size();i++) {
			
		}
		String vmCPU = Parameters.Result_file_location + "VM list CPU utlization.txt";
		try	{
			FileOutputStream fos1 = new FileOutputStream(vmCPU,false);  //Write the VM CPU to txt file
			OutputStreamWriter os1 = new OutputStreamWriter(fos1,"utf-8");
			for(int i=0;i<vmList.size();i++) {
				VM vm = vmList.get(i);
				for(int j=0;j<Parameters.assumedExTime;j++) {
					os1.append(i+"   "+"    "+vm.getCpuUtilize(j)+"");
				}
				os1.append("\r\n");
			}
			os.flush();
