...
public class Qlearning {
	...
	private void initActivePM(List<VM> vmList, List<PM> pmList, double currentTime) {
		...
		if(pmList.size() == 0) {
			PM newPM = creatComponents.createMinPowerPM(vmList.get(0), currentTime, vmList.get(0).getCpuUtilize(timeNo));
			pmList.add(newPM);
		}
	}
	...
	private int chooseAction(int state) {
		...
		if(!suitPM) {
			PM newPM = creatComponents.createMinPowerPM(vmList.get(state), currentTime, vmList.get(0).getCpuUtilize(timeNo));
			pmList.add(newPM);
	...
	public int[] getBestPMIndex() {
		...
		for (int i = 0; i < maxTrainStep; i++) {//Total training episode
			...
			if(Parameters.vmNum == 1000 && Parameters.arrivalLamda == 0.8 && vmList.size() == 12) {
	...
	public void fitness(int trainNum, Map<Integer, double[]> qTable) {
		...
		try	{
			...
			for(int i=0;i<pmList.size();i++){
				...
				double energyofPM = powerConsumpofPM*(currentTime-pm.getStartTime());
