...
public class OSER implements Scheduler {
	//Active and used PMs
		...
		while (arrivedVMNum < vmList.size() || finishedVMNum < vmList.size()) {
			...
			for (int i = arrivedVMNum; i < vmList.size(); i++) {
				if (vmList.get(i).getArriveTime() == currentTime) {
					...
					System.out.println("OSER--VM #" + vmList.get(i).getId() + " arriving at time:" + currentTime + " finish time: " + vmList.get(i).getFinishTime());
				} 
				else {
			...
			System.out.println("OSER--CurrentTime: " +currentTime + " timeNo: " + timeNo + " arrivedVMNum : " + arrivedVMNum+ " finishedVMNum :" + finishedVMNum);
			currentTime++;
		} //End while, the total loop finished when the vmList is empty
		
		System.out.println("OSER--All VMs in the vmlist have been scheduled at time: " + currentTime);

		//Check schedule results
		Check.checkUnfinishVMAndPM(vmList, activePMList, usedPMList);

		//Calculate the experimental results
		ExperimentResult.calculateExperimentResult("OSER", vmList, usedPMList, migrateCount,totalScheduleTime);
	...
	private void scheduleVMList(List<VM> vmList,List<PM> pmList) {
		...
		if (vmList.size() != 0) {
			...
			for (int i = 0; i < vmList.size(); i++) {
				...
				//Add the migrate count
				if(assignVM.getLastPM() != null && assignedPM != assignVM.getLastPM() ) {
					migrateCount++;
	...
	private void executeVMOnPM(List<PM> pmList) {
		...
		for (int i = 0; i < pmList.size(); i++) { //Get each PM, cannot remove the finish PM at here
			...
			for (int j = 0; j < vmList.size(); j++) { //Get each VM on a PM
				...
				if (vm.getFinishTime() <= currentTime) {
					...
					finishVM.add(vm);
					finishedVMNum++;
					System.out.println("OSER--VM #" + vm.getId() + " with finish time: " + vm.getFinishTime()
		...
		for (PM pm : finishPM) {
			System.out.println("OSER--Idle PM #" + pm.getId() + " is moved from activePMList to userPMList");
		}
		finishPM.clear();
	}
	...
	private void migrateVM(List<PM> pmList) {
		System.out.println("*************OSER start migrate***********");
		overloadPMList.clear();
		appropriatePMList.clear();
		lowloadPMList.clear();
		migrateVMList.clear();
