...
/**Z. Zhou, J. Abawajy, M. Chowdhury, et al,
 *  “Minimizing SLA violation and power consumption in Cloud data centers using adaptive energy-aware algorithms[J]. 
 *  Future Generation Computer Systems, 2018, 86: 836-850.”
 *  */
public class KMIMPCU  implements Scheduler {
	//Active and used PMs
	private List<PM> activePMList;
	private List<PM> usedPMList;

	//Two PMs group in activePMLists
	private List<PM> lessLoadedPMList; //Used for assign VMs when the PMs are all less loaded
	private List<PM> littleLoadedPMList; //serverListOne
	private List<PM> normallyLoadedPMList; //serverListTwo
	private List<PM> overloadedPMList;//Used for migrate VMs when the PMs are all over loaded
	
	//Three thresholds of PM's CPU
	private double ta,tb,tc;
	...
	public void schedule(List<VM> vmList) {
		...
		while (arrivedVMNum < vmList.size() || finishedVMNum < vmList.size()) {
			...
			for (int i = arrivedVMNum; i < vmList.size(); i++) {
				if (vmList.get(i).getArriveTime() == currentTime) {
					...
					System.out.println("KMIMPCU--VM #" + vmList.get(i).getId() + " arriving at time:" + currentTime
			...
			System.out.println("KMIMPCU--CurrentTime: " + currentTime + " timeNo: " + timeNo + " arrivedVMNum : "
		...
		System.out.println("KMIMPCU--All VMs in the vmlist have been scheduled at time: " + currentTime);

		//Check schedule results
		Check.checkUnfinishVMAndPM(vmList, activePMList, usedPMList);

		//Calculate the experimental results
		ExperimentResult.calculateExperimentResult("KMIMPCU", vmList, usedPMList, migrateCount, totalScheduleTime);

		//Clear the lists
		vmList.clear();
		activePMList.clear();
		usedPMList.clear();
		lessLoadedPMList.clear();
		littleLoadedPMList .clear();
		normallyLoadedPMList.clear();
		overloadedPMList.clear();
		...
		while (vmList.size() != 0) {
			...
			for(PM pm : pmList) {
				if(pm.isSuitableForVM(vm,vm.getCpuUtilize(timeNo))) {
					double cpuUtilize = pm.getCpuUtilize()+vm.getCpuUtilize(timeNo);
					if(cpuUtilize>=ta && cpuUtilize<=tb) {
						...
						double firstSLABefore = calculateSLATime(null,pm);//Get the first SAL;
						double firstSLAAfter = calculateSLATime(vm,pm);//Get the first SAL
						double firstSLA = firstSLAAfter - firstSLABefore;
						
						double secondSLABefore = calculateSLAMetric(null,pm);//Get the second SAL;
						double secondSLAAfter = calculateSLAMetric(vm,pm);//Get the second SAL
						double secondSLA = secondSLAAfter - secondSLABefore;
						
						double sla = firstSLA * secondSLA;
						double ef = 1/(powerConsumDiff*sla);
	...
	private void executeVMOnPM(List<PM> pmList) {
		...
		for (int i = 0; i < pmList.size(); i++) { // Get each PM, cannot remove the finish PM at here
			...
			for (int j = 0; j < vmList.size(); j++) { // Get each VM on a PM
				...
				if (vm.getFinishTime() <= currentTime) {
					...
					finishVM.add(vm);
					finishedVMNum++;
					System.out.println("KMIMPCU--VM #" + vm.getId() + " with finish time: " + vm.getFinishTime()
		...
		for (PM pm : finishPM) {
			System.out.println("KMIMPCU--Idle PM #" + pm.getId() + " is moved from activePMList to userPMList");
		}
		finishPM.clear();
	}
	...
	private void migrateVM(List<PM> pmList) {
		...
		//1.Get the three thresholds by KMIR algorithm, and divide the active PMs
		lessLoadedPMList.clear();
		overloadedPMList.clear();
		
		for(PM pm : pmList) {
			//Less loaded PMs
			if(pm.getCpuUtilize() < ta ) {
				lessLoadedPMList.add(pm);
			}
			//Overloaded PMs
			if(pm.getCpuUtilize() >= tc) {
				overloadedPMList.add(pm);
			}
		}
		
		//2.Migrate VMs of the lessLoaded PM list 
		while(lessLoadedPMList.size()>1) { //Migrate VM only there has more than one less loaded PM
			PM pm = lessLoadedPMList.get(lessLoadedPMList.size()-1); //Chose the last PM when assign object PM maybe the first one
			...
			lessLoadedPMList.remove(pm);	
		}
		
		//3.Migrate VMs of the heailyloaded PM list
		for(PM pm : overloadedPMList) {
			double pmCPUUtilize = pm.getCpuUtilize();
				
			while(pmCPUUtilize >= tc) {
				double minMPCP = Double.MAX_VALUE;
				VM selectedVM = null;
				for(VM vm : pm.getActiveVMList()) {
					double tempMPCP = (vm.getRequestedPes()/pm.getTotalPes())*(vm.getRequestedRam()/pm.getTotalRam());
					if(tempMPCP < minMPCP) {
						minMPCP = tempMPCP;
						selectedVM = vm;
					}
		...
		if(migrateVMList.size() > 0) {
			...
			System.out.println("*************KMIMPCU start migrate***********");
	...
	/**Get the three thresholds of PM, K-Means-Mad-IQR */
	private void KMIR(List<VM> vmList, List<PM> pmList) {
		...
		if(pastCPUUtilization.size() >= 12) {
			...
			//Eqs.3-5
			ta = 0.5*(1-Parameters.d *iqr); //Set d as 1.0
			tb = 0.9*(1-Parameters.d *iqr); //Set d as 1.0
			tc = (1-Parameters.d *iqr); //Set d as 1.0
		}
		else {
	...
	//Calculate the SLA Violation time of an active PM before/after assigned a VM
	private double calculateSLATime(VM vm, PM pm) {
	...
	//Calculate the performance degradation of a PM caused by VM migration before/after assigned a VM
		private double calculateSLAMetric(VM vm, PM pm) {
			int slaVT = 0; //The SLA violation times
			double cpuUtilize = 0;
			int pesUtilize = 0;
			double ramUtilize = 0;
